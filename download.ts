...
const resumeContent: any = document.getElementById(
  "resume-content"
) as HTMLDivElement;
...
form.addEventListener("submit", (e) => {
  ...
  const maritalStatusSelect = document.getElementById(
    "maritalStatus"
  ...
  if (dateOfBirth > cutoffDate) {
    alert(
      "📅 Your date of birth is too recent. Please enter a valid date of birth that makes you eligible!"
    );
    return;
  }
  ...
  if (age < 10) {
    ...
    alert(
      "🎉 Wow, that’s impressive! But let's be realistic - age should not be greater than 90."
    );
    return;
  }
  ...
  const computerSkills = (
    document.getElementById("computerSkills") as HTMLInputElement
  ).value;
